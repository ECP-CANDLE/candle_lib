...
def test_parse_from_dictlist(testobj):
    parser_with_conf = candle.parse_from_dictlist(testobj.config,
                                                  testobj.parser)
...
def test_indirect_finalize_parameters(testobj):
    """Cannot check directly the function since it requires proper command line
    options.
