...
def test_absadapt_callback():
    ...
    try:
        candle.AbstentionAdapt_Callback(
...
def test2_absadapt_callback():
    ...
    alpha0 = 0.1
    cbk = candle.AbstentionAdapt_Callback(
        acc_monitor="val_abstention_acc", abs_monitor="val_abstention", alpha0=alpha0
    )
    callbacks = [cbk]
    sgd = optimizers.SGD(lr=1e-4, decay=1e-6, momentum=0.9, nesterov=True)
    try:
        model.compile(
            loss=candle.abstention_loss(cbk.alpha, mask),
            optimizer=sgd,
            metrics=[
                "acc",
...
def test1_add_model_output(testobj):
    ...
    num_add = 1
    model = candle.add_model_output(
        testobj.model, mode="abstain", num_add=num_add, activation="sigmoid"
    )
...
def test_contamination_loss(testobj):
    ...
    contmloss = candle.contamination_loss(
        testobj.nout_reg_cont, Tk, a, sigmaSQ, gammaSQ
    )
    try:
    ...
    try:
        model.compile(
            loss=candle.contamination_loss(
                nout, cbk.T_k, cbk.a, cbk.sigmaSQ, cbk.gammaSQ
            ),
            optimizer=sgd,
            metrics=[
        ...
        model.fit(
            xtrain, ytrain_augmented, batch_size=10, epochs=1, callbacks=callbacks
        )
