...
def test_quantile_normalization():
    ...
    assert np.round(a=np.max(third_quartile) - np.min(third_quartile), decimals=2) == 0.01
...
def test_generate_cross_validation_partition():
    generated = candle.generate_cross_validation_partition(range(5), n_folds=5, n_repeats=2, portions=None, random_seed=0)
    expected = [
        [[2], [0, 1, 3, 4]],
        [[0], [1, 2, 3, 4]],
        [[1], [0, 2, 3, 4]],
    ...
    eval_1 = [isArrayEqual(generated[i][0], expected[i][0]) for i in range(len(generated))]
    eval_2 = [isArrayEqual(generated[i][1], expected[i][1]) for i in range(len(generated))]
