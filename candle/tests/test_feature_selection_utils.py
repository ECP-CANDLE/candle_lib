...
def test_decorrelated_features():
    data = load_data()
    selected = candle.select_decorrelated_features(
        data, threshold=None, random_seed=None
    )
    assert all([a == b for a, b in zip(selected, [0, 1, 2, 3, 4, 5, 6, 9])])
