...
def test_select_features_by_missing_values():
    ...
    selected = candle.select_features_by_missing_values(data.values, threshold=0.3)
    assert all([a == b for a, b in zip(selected, [0, 1, 2, 3, 4, 5, 6, 9])])


def test_select_features_by_variation():
    data = load_data()
    selected = candle.select_features_by_variation(data, variation_measure='var', threshold=100, portion=None, draw_histogram=False)
    assert all([a == b for a, b in zip(selected, [0, 3, 5])])

    selected = candle.select_features_by_variation(data, variation_measure='std', portion=0.2)
    assert all([a == b for a, b in zip(selected, [0, 5])])


def test_decorrelated_features():
    data = load_data()
    selected = candle.select_decorrelated_features(data, threshold=None, random_seed=None)
    assert all([a == b for a, b in zip(selected, [0, 1, 2, 3, 4, 5, 6, 9])])

    selected = candle.select_decorrelated_features(data, method='spearman', threshold=0.8, random_seed=10)
