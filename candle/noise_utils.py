        ...
        if params['label_noise']:
            ...
            if params['noise_correlated']:
                labels, y_noise_gen = label_flip_correlated(
                    labels, params['label_noise'], data, params['feature_col'],
...
def label_flip_correlated(y_data_categorical, y_noise_level, x_data, col_ids,
                          threshold):
        ...
        for i in range(0, y_data_categorical.shape[0]):
            if x_data[i][col_id] > threshold:
                if random.random() < y_noise_level:
                    ...
                    flip_count += 1
                    for j in range(y_data_categorical.shape[1]):
                        y_data_categorical[i][j] = int(
                            not y_data_categorical[i][j])
                    print(i, y_data_categorical[i][:])

        y_noise_generated = float(flip_count) / float(
...
def add_column_noise(x_data,
    ...
    for col_id in col_ids:
        ...
        if noise_type == 'gaussian':
            ...
            train_noise = np.random.uniform(-1.0 * scale,
                                            scale,
                                            size=x_data.shape[0])
        print(train_noise)
    ...
    for row_id in row_ids:
        ...
        if random.random() < y_noise_level:
            ...
            for col_id in col_ids:
                ...
                x_data[
                    row_id,
                    col_id] = 1.0 * x_data[row_id, col_id] + 1.0 * train_noise
                print(x_data[row_id, col_id])
